...
export default function ParticipantsTab() {
  ...
  const handleUploadCSV = async (e) => {
    ...
    setIsUploading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_NODE_URL || 'https://orchestr-backend-8u5k.onrender.com'}/api/admin/upload-roster`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleAddParticipant = async () => {
    ...
    const skill = addForm.skill.trim();
    const institution = addForm.institution.trim();

    if (!name || !email || !skill || !institution) {
