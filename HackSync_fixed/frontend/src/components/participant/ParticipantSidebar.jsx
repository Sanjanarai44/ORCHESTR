...
export default function ParticipantSidebar({ activeSection, onNavClick, isOpen, onClose }) {
  const links = [
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-full flex flex-col py-8 bg-[#012d1d] text-white w-64 shadow-xl z-50 transition-transform duration-300
        ...
        <nav className="flex-1 flex flex-col gap-1">
          {links.map((link) => (
            <a
              key={link.id}
              href={`#${link.id}`}
              onClick={(e) => { onNavClick(e, link.id); onClose?.(); }}
