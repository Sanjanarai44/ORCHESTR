  ...
  return (
    <div className="bg-amber-50 border border-amber-300 rounded-xl px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-bold text-amber-900">{count} team{count !== 1 ? "s" : ""} pending your approval</p>
          <p className="text-xs text-amber-700 mt-0.5">Generated but not published. Participants won't see assignments until you approve.</p>
        </div>
      </div>
      <div className="flex items-center gap-3 flex-shrink-0">
        <button onClick={onDismiss} className="text-xs font-semibold text-amber-700 hover:text-amber-900 px-3 py-2 rounded-lg hover:bg-amber-100 transition-colors">
          Review First
        </button>
...
export default function AdminDashboard({ eventConfig, eventId = 1, organizer, onBack, onLogout }) {
  const [activeTab, setActiveTab] = useState("Dashboard");
  const [showBanner, setShowBanner] = useState(true);
  const [refreshKey, setRefreshKey] = useState(0);

  const handleApproved = () => { setShowBanner(false); setRefreshKey(k => k + 1); };

  return (
    <AdminLayout activeTab={activeTab} setActiveTab={setActiveTab} eventConfig={eventConfig}
      organizer={organizer} onBack={onBack} onLogout={onLogout}>

      {activeTab === "Dashboard" && (
        <div className="space-y-6">
          {showBanner && (
            <ApprovalBanner eventId={eventId} onApprove={handleApproved} onDismiss={() => setActiveTab("Teams")} />
          )}
      ...
      {activeTab === "Participants" && <ParticipantsTab eventConfig={eventConfig} eventId={eventId} />}
      {activeTab === "Teams" && <TeamsTab eventConfig={eventConfig} eventId={eventId} onTeamsGenerated={() => setShowBanner(true)} />}
      {activeTab === "Judges" && <JudgesTab eventConfig={eventConfig} eventId={eventId} />}
      {activeTab === "Evaluations" && <EvaluationsTab eventConfig={eventConfig} eventId={eventId} />}
    </AdminLayout>
  );
}