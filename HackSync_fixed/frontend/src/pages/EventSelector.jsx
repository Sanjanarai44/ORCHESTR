...
function EventCard({ event, onOpen, onDelete }) {
  ...
  const typeColor =
    typeColors[event.event_type?.toLowerCase()] ||
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex-1 min-w-0">
            <h3 className="font-extrabold text-[#012d1d] text-lg leading-tight truncate">
              {event.name}
            </h3>
          ...
          <span
            className={`text-[10px] font-bold px-2.5 py-1 rounded-full border capitalize flex-shrink-0 ${typeColor}`}
          >
            {event.event_type || "event"}
          </span>
        </div>

        {/* Stats — read from config, no hardcoded hackathon defaults */}
        <div className="grid grid-cols-2 gap-3 mb-4">
          {[
            ...
            { icon: "flag",        label: "Stages",       val: stages.length > 0 ? stages.length : "—" },
          ].map((s, i) => (
            <div key={i} className="bg-[#F5F3F0] rounded-xl p-3 flex items-center gap-2">
              <span className="material-symbols-outlined text-[#012d1d]/50 text-[16px]">
                {s.icon}
              </span>
              <div>
        ...
        {/* Scoring criteria — only shown if the event actually has them */}
        {cfg.scoring_criteria?.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {cfg.scoring_criteria.slice(0, 3).map((c, i) => (
              <span
                key={i}
                className="text-[10px] bg-[#F5F3F0] text-[#5a6672] px-2 py-0.5 rounded-full capitalize"
              >
                {c}
              </span>
            ))}
            {cfg.scoring_criteria.length > 3 && (
              <span className="text-[10px] text-[#9aa5ae]">
                +{cfg.scoring_criteria.length - 3} more
              </span>
            )}
          </div>
        )}
...
export default function EventSelector({ organizer, onSelectEvent, onNewEvent, onLogout }) {
  ...
  const fetchEvents = async () => {
    setLoading(true);
    try {
      // Uses eventsApi → AI backend (/events) which is the correct endpoint
      const data = await eventsApi.getAll(organizer.id);
  ...
  const handleDelete = async (event) => {
    ...
    try {
      // Uses eventsApi → AI backend (/events/:id DELETE)
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {events.map((event) => (
              <div
                key={event.id}
                className={deleting === event.id ? "opacity-40 pointer-events-none" : ""}
              >
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-[#5a6672] mt-1 max-w-[160px]">
                  Describe it in plain language — AI does the rest
                </p>
              </div>
            </button>
          </div>
